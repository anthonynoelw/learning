...
function getWebSocketServer() {
  ...
  } else if (window.location.host === "[::1]:8000") {
  ...
  websocket.addEventListener("message", ({ data }) => {
    ...
    switch (event.type) {
      case "init":
        ...
        // Open in a new tab so Player 1's WebSocket stays alive.
        document.querySelector(".join").href = "?join=" + event.join;
        document.querySelector(".join").target = "_blank";
        document.querySelector(".watch").href = "?watch=" + event.watch;
        document.querySelector(".watch").target = "_blank";
        break;
      case "play":
